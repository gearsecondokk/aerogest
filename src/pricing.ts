...
export async function estimateCost(model: VideoModel, options: Options): Promise<CostEstimate> {
  ...
  // L'API de tarifs de fal ne connaît que les endpoints fal : pour BytePlus ou
  // TopView, « endpoint » est un identifiant maison et l'appel répondrait 400.
  const live = model.provider && model.provider !== "fal" ? null : await fetchLiveUnitPrice(model.endpoint);
